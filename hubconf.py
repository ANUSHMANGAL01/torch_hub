...
def _train(dataloader, model, loss_fn, optimizer):
  ...
  for batch, (X,y) in enumerate(dataloader):
    ...
    pred = model(X)
    loss = loss_fn(pred, y) # Cross Entropy one define earlier
    ...
    if batch%100 == 0 : # just for book keeping
...
def train(train_dataloader, test_dataloader, model, loss_fn, optimizer, epochs=6):
